...
public class testAPITask {
	...
	public void getSecondMethod ()
	{
		...
		JSONArray parselist = new JSONArray(response.getBody().print());
		net.minidev.json.JSONArray expectedValues = readJSONfile();
		System.out.println("length = "+parselist.length());
		
		for(int i=0;i<parselist.length();i++){
			Gson gson = new Gson();
			JsonParser parser = gson.fromJson(parselist.get(i).toString(), JsonParser.class);
			JsonParser expectedParser = gson.fromJson(expectedValues.get(i).toString(), JsonParser.class);
			assertValues(expectedParser, parser);
		}
		assertEquals(200, response.getStatusCode());
